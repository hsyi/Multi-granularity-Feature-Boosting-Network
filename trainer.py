...
class Trainer():
    def __init__(self, args, model, loss, loader, ckpt):
        ...
        self.division_phase = 0

        if args.model.lower() == 'division_net':
            division_step = args.division_step.split('_')
            self.division_step = list(map(lambda x: int(x), division_step))
        ...
        if args.load != '':
            ...
            for _ in range(len(ckpt.loss_log)): self.scheduler.step()

    def train(self):
        self.scheduler.step()
        self.loss.step()
        epoch = self.scheduler.last_epoch + 1
        if self.args.model.lower() == 'division_net':
            if self.division_phase < 2 and epoch == self.division_step[self.division_phase]:
                self.division_phase += 1
                print("model division")
                self.model.net_division(self.division_phase)
        if self.args.lr_scheduler.find('warmup') >= 0:
            lr = self.scheduler.update(epoch)
        ...
        for batch, (inputs, labels) in enumerate(self.train_loader):
            ...
            loss = self.loss(outputs, labels, division_phase = self.division_phase)
            loss.backward()
            self.optimizer.step()
    ...
    def test(self):
        epoch = self.scheduler.last_epoch + 1
        self.ckpt.write_log('[INFO] Save Model')
        self.model.eval()
    ...
    def extract_feature(self, loader):
        ...
        for (inputs, labels) in loader:
            if self.args.model.lower() == "erenet":
                ff = torch.FloatTensor(inputs.size(0), 256*7).zero_()
            elif self.args.model.lower() == 'division_net':
                if self.division_phase == 0:
                    ff = torch.FloatTensor(inputs.size(0), 256).zero_()
                elif self.division_phase == 1:
                    ff = torch.FloatTensor(inputs.size(0), 256*4).zero_()
                else:
                    ff = torch.FloatTensor(inputs.size(0), 256*8).zero_()
            else:
